  ...
  <div class="app-container">
    ...
    >
      ...
      <el-table-column label="操作" align="center" class-name="small-padding fixed-width" width="220">
        <template #default="scope">
          <el-button link type="primary" icon="Edit" @click="handleUpdate(scope.row)" v-hasPermi="['basic:unit:edit']">修改</el-button>
          <el-button link type="primary" icon="Plus" @click="handleAdd(scope.row)" v-hasPermi="['basic:unit:add']">新增</el-button>
          <el-button link type="primary" icon="Delete" @click="handleDelete(scope.row)" v-hasPermi="['basic:unit:remove']">删除</el-button>
        </template>
      </el-table-column>
    </el-table>
...
<script setup name="BasicUnit">
import { listUnit, getUnit, delUnit, addUnit, updateUnit } from "@/api/basic/unit"
...
function getList() {
  ...
  listUnit(queryParams.value).then(response => {
    unitList.value = proxy.handleTree(response.data, "id", "parentId")
    loading.value = false
  })
}
...
async function handleUpdate(row) {
  ...
  getUnit(row.id).then(response => {
    form.value = response.data
    open.value = true
    title.value = "修改基础数据-单位"
  })
}
