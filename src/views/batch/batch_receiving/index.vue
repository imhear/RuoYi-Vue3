<template>
  <div class="app-container">
    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button
          ...
          icon="Plus"
          @click="handleAdd"
          v-hasPermi="['batch:batch_receiving:add']"
        >新增</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="single"
          @click="handleUpdate"
          v-hasPermi="['batch:batch_receiving:edit']"
        >修改</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="multiple"
          @click="handleDelete"
          v-hasPermi="['batch:batch_receiving:remove']"
        >删除</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          icon="Download"
          @click="handleExport"
          v-hasPermi="['batch:batch_receiving:export']"
        >导出</el-button>
      </el-col>
      <right-toolbar v-model:showSearch="showSearch" @queryTable="getList"></right-toolbar>
    </el-row>

    <el-table v-loading="loading" :data="batch_receivingList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="操作" align="center" class-name="small-padding fixed-width">
        <template #default="scope">
          <el-button link type="primary" icon="View" @click="handleViewData(scope.row)" v-hasPermi="['batch:batch_receiving:query']">详情</el-button>
          <el-button link type="primary" icon="Edit" @click="handleUpdate(scope.row)" v-hasPermi="['batch:batch_receiving:edit']">修改</el-button>
          <el-button link type="primary" icon="Delete" @click="handleDelete(scope.row)" v-hasPermi="['batch:batch_receiving:remove']">删除</el-button>
        </template>
      </el-table-column>
    </el-table>
    ...
    <!-- 领料单详情抽屉 -->
    <batch_receiving-view-drawer ref="batch_receivingViewRef" />
    <!-- 添加或修改领料单对话框 -->
    <el-dialog :title="title" v-model="open" width="500px" append-to-body>
      <el-form ref="batch_receivingRef" :model="form" :rules="rules" label-width="100px">
        <el-row>
          <el-col :span="24">
            <el-form-item label="关联排产计划ID" prop="planId">
...
<script setup name="Batch_receiving">
import { listBatch_receiving, getBatch_receiving, delBatch_receiving, addBatch_receiving, updateBatch_receiving } from "@/api/batch/batch_receiving"
import Batch_receivingViewDrawer from "./view"

const { proxy } = getCurrentInstance()

const batch_receivingList = ref([])
...
function getList() {
  loading.value = true
  listBatch_receiving(queryParams.value).then(response => {
    batch_receivingList.value = response.rows
...
function reset() {
  ...
  proxy.resetForm("batch_receivingRef")
}
...
function handleUpdate(row) {
  ...
  getBatch_receiving(_receivingId).then(response => {
...
/** 提交按钮 */
function submitForm() {
  proxy.$refs["batch_receivingRef"].validate(valid => {
    if (valid) {
      if (form.value.receivingId != null) {
        updateBatch_receiving(form.value).then(() => {
          proxy.$modal.msgSuccess("修改成功")
          open.value = false
          getList()
        })
      } else {
        addBatch_receiving(form.value).then(() => {
...
function handleDelete(row) {
  ...
  proxy.$modal.confirm('是否确认删除领料单编号为"' + _receivingIds + '"的数据项？').then(function() {
    return delBatch_receiving(_receivingIds)
  }).then(() => {
    getList()
...
function handleViewData(row) {
  proxy.$refs["batch_receivingViewRef"].open(row.receivingId)
}

/** 导出按钮操作 */
function handleExport() {
  proxy.download('batch/batch_receiving/export', {
    ...queryParams.value
  }, `batch_receiving_${new Date().getTime()}.xlsx`)
}
