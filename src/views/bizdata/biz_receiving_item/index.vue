<template>
  <div class="app-container">
    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button
          ...
          icon="Plus"
          @click="handleAdd"
          v-hasPermi="['bizdata:biz_receiving_item:add']"
        >新增</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="single"
          @click="handleUpdate"
          v-hasPermi="['bizdata:biz_receiving_item:edit']"
        >修改</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="multiple"
          @click="handleDelete"
          v-hasPermi="['bizdata:biz_receiving_item:remove']"
        >删除</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          icon="Download"
          @click="handleExport"
          v-hasPermi="['bizdata:biz_receiving_item:export']"
        >导出</el-button>
      </el-col>
      <right-toolbar v-model:showSearch="showSearch" @queryTable="getList"></right-toolbar>
    </el-row>

    <el-table v-loading="loading" :data="biz_receiving_itemList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="操作" align="center" class-name="small-padding fixed-width">
        <template #default="scope">
          <el-button link type="primary" icon="Edit" @click="handleUpdate(scope.row)" v-hasPermi="['bizdata:biz_receiving_item:edit']">修改</el-button>
          <el-button link type="primary" icon="Delete" @click="handleDelete(scope.row)" v-hasPermi="['bizdata:biz_receiving_item:remove']">删除</el-button>
        </template>
      </el-table-column>
    </el-table>
    ...
    <el-dialog :title="title" v-model="open" width="500px" append-to-body>
      <el-form ref="biz_receiving_itemRef" :model="form" :rules="rules" label-width="100px">
        <el-row>
          <el-col :span="24">
            <el-form-item label="关联领料单主键" prop="receivingId">
...
<script setup name="Biz_receiving_item">
import { listBiz_receiving_item, getBiz_receiving_item, delBiz_receiving_item, addBiz_receiving_item, updateBiz_receiving_item } from "@/api/bizdata/biz_receiving_item"

const { proxy } = getCurrentInstance()

const biz_receiving_itemList = ref([])
...
function getList() {
  loading.value = true
  listBiz_receiving_item(queryParams.value).then(response => {
    biz_receiving_itemList.value = response.rows
...
function reset() {
  ...
  proxy.resetForm("biz_receiving_itemRef")
}
...
function handleUpdate(row) {
  ...
  getBiz_receiving_item(_itemId).then(response => {
...
/** 提交按钮 */
function submitForm() {
  proxy.$refs["biz_receiving_itemRef"].validate(valid => {
    if (valid) {
      if (form.value.itemId != null) {
        updateBiz_receiving_item(form.value).then(() => {
          proxy.$modal.msgSuccess("修改成功")
          open.value = false
          getList()
        })
      } else {
        addBiz_receiving_item(form.value).then(() => {
...
function handleDelete(row) {
  ...
  proxy.$modal.confirm('是否确认删除领料单子表编号为"' + _itemIds + '"的数据项？').then(function() {
    return delBiz_receiving_item(_itemIds)
  }).then(() => {
    getList()
...
function handleExport() {
  proxy.download('bizdata/biz_receiving_item/export', {
    ...queryParams.value
  }, `biz_receiving_item_${new Date().getTime()}.xlsx`)
}
