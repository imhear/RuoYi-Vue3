<!--
  灌装包材处理记录2 检查对话框（纯只读 + 检查按钮）
  基线：在查看对话框纯内容组件基础上改造
  改造：签名行增加检查按钮，所有字段只读
  交互：完全被动模式，emit 数据给父组件处理
  Step5/Step6 均支持检查，按钮显隐由 s5InspectShowHide / s6InspectShowHide 控制
-->
<template>
      ...
      <table class="row-table step-table-native" cellspacing="0" cellpadding="0">
        ...
        <tr style="height: 100px;">
          ...
          <td class="td-no-padding">
            <table class="inner-fill-table step5-inner" style="border-collapse: collapse; font-size: 14px; width: 100%; height: 100%;">
              <tr style="height: 25px;">
                <td class="split-td">
                  <span class="str-left">1）{{ form.s5MaterialName1 || '' }}</span>
                  <span class="str-right">数量：{{ form.s5MaterialQty1 || '' }} ；</span>
                </td>
              </tr>
              <tr style="height: 25px;">
                <td class="split-td">
                  <span class="str-left">2）{{ form.s5MaterialName2 || '' }}</span>
                  <span class="str-right">数量：{{ form.s5MaterialQty2 || '' }} ；</span>
                </td>
              </tr>
              <tr style="height: 25px;">
                <td class="split-td">
                  <span class="str-left">3）{{ form.s5MaterialName3 || '' }}</span>
                  <span class="str-right">数量：{{ form.s5MaterialQty3 || '' }} ；</span>
                </td>
              </tr>
              <tr style="height: 25px;">
                <td class="split-td">
                  <span class="str-left">4）{{ form.s5MaterialName4 || '' }}</span>
                  <span class="str-right">数量：{{ form.s5MaterialQty4 || '' }} ；</span>
                </td>
              </tr>
            </table>
          </td>
          <td class="td-sign">
            ...
            <!-- 检查人未提交 且 显示开关开启(值为0)时，显示检查按钮 -->
            <div v-if="!form.s5InspectorTime && form.s5InspectShowHide === '0'" style="margin-top: 8px;">
            <el-button type="warning" size="small" @click="handleInspectStep5">检查</el-button>
            </div>
          </td>
        </tr>
      </table>

      <!-- ===== Step6 生产过程不合格物料处理（只读 + 检查按钮） ===== -->
      <table class="row-table step-table-native" cellspacing="0" cellpadding="0">
        <colgroup>
          <col style="width: 102px;">
          <col style="width: 125px;">
        ...
        <tr style="height: 28px;">
          ...
          <td class="td-sign" rowspan="10">
            ...
            <!-- 检查人未提交 且 显示开关开启(值为0)时，显示检查按钮 -->
            <div v-if="!form.s6InspectorTime && form.s6InspectShowHide === '0'" style="margin-top: 8px;">
            <el-button type="warning" size="small" @click="handleInspectStep6">检查</el-button>
            </div>
          </td>
        </tr>
...
/** Step5 检查：emit 给父组件处理 */
function handleInspectStep5() {
  emit('step5Inspect', { recordId: currentRecordId.value })
}

/** Step6 检查：emit 给父组件处理 */
function handleInspectStep6() {
  emit('step6Inspect', { recordId: currentRecordId.value })
}
...
const emit = defineEmits(['step5Inspect', 'step6Inspect', 'submit'])
