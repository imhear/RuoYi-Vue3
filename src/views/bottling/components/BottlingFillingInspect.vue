<!--
  灌装生产记录1 检查对话框（全部只读，签名列可点击检查按钮）
  宽度 300mm，完全被动模式，子组件 emit 数据给父组件
-->
<template>
  <el-dialog v-model="visible" title="灌装生产记录检查" width="300mm" append-to-body @closed="handleClosed">
      ...
      <table class="row-table step-table-native" cellspacing="0" cellpadding="0">
        ...
        <tr style="height: 120px;">
          ...
          <td class="td-sign">
            ...
            <!-- 检查人：显示签名或检查按钮 -->
            <div v-if="form.s1InspectShowHide !== '1'" style="margin-top: 8px;">
              检查人：{{ form.s1Inspector || '' }}
              <div v-if="form.s1InspectShowHide !== '1' && !form.s1InspectorTime" style="margin-top: 4px;">
                <el-button type="warning" size="small" @click="handleStep1Inspect">检查</el-button>
              </div>
              <div v-if="form.s1InspectorTime" style="color: gray; font-size: 12px;">{{ form.s1InspectorTime.substring(0, 16) }}</div>
            </div>
          </td>
        </tr>
      </table>
      ...
      <table class="row-table step-table-native" cellspacing="0" cellpadding="0">
        ...
        <tr style="height: 100px;">
          ...
          <td class="td-sign">
            ...
            <!-- 检查人：显示签名或检查按钮 -->
            <div v-if="form.s2InspectShowHide !== '1'" style="margin-top: 8px;">
              检查人：{{ form.s2Inspector || '' }}
              <div v-if="form.s2InspectShowHide !== '1' && !form.s2InspectorTime" style="margin-top: 4px;">
                <el-button type="warning" size="small" @click="handleStep2Inspect">检查</el-button>
              </div>
              <div v-if="form.s2InspectorTime" style="color: gray; font-size: 12px;">{{ form.s2InspectorTime.substring(0, 16) }}</div>
            </div>
          </td>
        </tr>
      </table>
      ...
      <table class="row-table step-table-native" cellspacing="0" cellpadding="0">
        ...
        <tr style="height: auto;">
          ...
          <td class="td-sign">
            ...
            <br/>
            <!-- 检查人：显示签名或检查按钮 -->
            <div v-if="form.s3InspectShowHide !== '1'" style="margin-top: 8px;">
              检查人：{{ form.s3Inspector || '' }}
              <div v-if="form.s3InspectShowHide !== '1' && !form.s3InspectorTime" style="margin-top: 4px;">
                <el-button type="warning" size="small" @click="handleStep3Inspect">检查</el-button>
              </div>
              <div v-if="form.s3InspectorTime" style="color: gray; font-size: 12px;">{{ form.s3InspectorTime.substring(0, 16) }}</div>
            </div>
          </td>
        </tr>
      </table>
...
// 定义 emit 事件，每个步骤检查单独事件
const emit = defineEmits(['step1Inspect', 'step2Inspect', 'step3Inspect'])
...
/** 检查 Step1：emit 事件，传递 recordId */
function handleStep1Inspect() {
  emit('step1Inspect', { recordId: form.recordId })
}

/** 检查 Step2 */
function handleStep2Inspect() {
  emit('step2Inspect', { recordId: form.recordId })
}

/** 检查 Step3 */
function handleStep3Inspect() {
  emit('step3Inspect', { recordId: form.recordId })
}
