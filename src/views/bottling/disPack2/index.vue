<template>
  <div class="app-container">
    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button
          ...
          icon="Plus"
          @click="handleAdd"
          v-hasPermi="['bottling:disPack2:add']"
        >新增</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="single"
          @click="handleUpdate"
          v-hasPermi="['bottling:disPack2:edit']"
        >修改</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="multiple"
          @click="handleDelete"
          v-hasPermi="['bottling:disPack2:remove']"
        >删除</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          icon="Download"
          @click="handleExport"
          v-hasPermi="['bottling:disPack2:export']"
        >导出</el-button>
      </el-col>
      <right-toolbar v-model:showSearch="showSearch" @queryTable="getList"></right-toolbar>
    </el-row>

    <el-table v-loading="loading" :data="disPack2List" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="操作" align="center" class-name="small-padding fixed-width">
        <template #default="scope">
          <el-button link type="primary" icon="Edit" @click="handleUpdate(scope.row)" v-hasPermi="['bottling:disPack2:edit']">修改</el-button>
          <el-button link type="primary" icon="Delete" @click="handleDelete(scope.row)" v-hasPermi="['bottling:disPack2:remove']">删除</el-button>
        </template>
      </el-table-column>
    </el-table>
    ...
    <el-dialog :title="title" v-model="open" width="500px" append-to-body>
      <el-form ref="disPack2Ref" :model="form" :rules="rules" label-width="100px">
        <el-row>
          <el-col :span="24">
            <el-form-item label="关联排产计划" prop="planId">
...
<script setup name="DisPack2">
import { listDisPack2, getDisPack2, delDisPack2, addDisPack2, updateDisPack2 } from "@/api/bottling/disPack2"

const { proxy } = getCurrentInstance()

const disPack2List = ref([])
...
function getList() {
  loading.value = true
  listDisPack2(queryParams.value).then(response => {
    disPack2List.value = response.rows
...
function handleUpdate(row) {
  ...
  getDisPack2(_recordId).then(response => {
...
/** 提交按钮 */
function submitForm() {
  proxy.$refs["disPack2Ref"].validate(valid => {
    if (valid) {
      if (form.value.recordId != null) {
        updateDisPack2(form.value).then(() => {
          proxy.$modal.msgSuccess("修改成功")
          open.value = false
          getList()
        })
      } else {
        addDisPack2(form.value).then(() => {
...
function handleExport() {
  proxy.download('bottling/disPack2/export', {
    ...queryParams.value
  }, `disPack2_${new Date().getTime()}.xlsx`)
}
