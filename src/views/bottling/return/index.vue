<template>
  <div class="app-container">
    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button
          ...
          icon="Plus"
          @click="handleAdd"
          v-hasPermi="['bottling:return:add']"
        >新增</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="single"
          @click="handleUpdate"
          v-hasPermi="['bottling:return:edit']"
        >修改</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="multiple"
          @click="handleDelete"
          v-hasPermi="['bottling:return:remove']"
        >删除</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          icon="Download"
          @click="handleExport"
          v-hasPermi="['bottling:return:export']"
        >导出</el-button>
      </el-col>
    ...
    <el-table v-loading="loading" :data="returnList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="退料单主键" align="center" prop="returnId" />
      <el-table-column label="关联排产计划" align="center" prop="planId" />
      <el-table-column label="车间" align="center" prop="workshop" />
      <el-table-column label="退料类型" align="center" prop="returnType" />
      <el-table-column label="退料日期" align="center" prop="returnDate" width="180">
      ...
      <el-table-column label="操作" align="center" class-name="small-padding fixed-width">
        <template #default="scope">
          <el-button link type="primary" icon="Edit" @click="handleUpdate(scope.row)" v-hasPermi="['bottling:return:edit']">修改</el-button>
          <el-button link type="primary" icon="Delete" @click="handleDelete(scope.row)" v-hasPermi="['bottling:return:remove']">删除</el-button>
        </template>
      </el-table-column>
    </el-table>
    ...
    <el-dialog :title="title" v-model="open" width="500px" append-to-body>
      <el-form ref="returnRef" :model="form" :rules="rules" label-width="100px">
        <el-row>
          <el-col :span="24">
            <el-form-item label="关联排产计划" prop="planId">
...
import { listReturn, getReturn, delReturn, addReturn, updateReturn } from "@/api/bottling/return"
...
function handleUpdate(row) {
  ...
  getReturn(_returnId).then(response => {
  ...
  proxy.$refs["returnRef"].validate(valid => {
    if (valid) {
      if (form.value.returnId != null) {
        updateReturn(form.value).then(() => {
          proxy.$modal.msgSuccess("修改成功")
          open.value = false
          getList()
        })
      } else {
        addReturn(form.value).then(() => {
...
function handleDelete(row) {
  ...
  proxy.$modal.confirm('是否确认删除退料单编号为"' + _returnIds + '"的数据项？').then(function() {
    return delReturn(_returnIds)
  }).then(() => {
    getList()
...
function handleExport() {
  proxy.download('bottling/return/export', {
    ...queryParams.value
