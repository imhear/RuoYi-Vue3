<template>
  <div class="app-container">
    <el-form :model="queryParams" ref="queryRef" :inline="true" v-show="showSearch" label-width="68px">
      ...
      <el-form-item label="所在分组" prop="groupName">
        <el-input
    ...
    <el-row :gutter="10" class="mb8">
      ...
      <!-- <el-col :span="1.5">
        <el-button
          ...
          icon="Plus"
          @click="handleAdd"
          v-hasPermi="['fill:formtemplate:add']"
        >新增</el-button>
      </el-col> -->
      <!-- <el-col :span="1.5">
        <el-button
          ...
          :disabled="single"
          @click="handleUpdate"
          v-hasPermi="['fill:formtemplate:edit']"
        >修改</el-button>
      </el-col> -->
      <!-- <el-col :span="1.5">
        <el-button
          ...
          :disabled="multiple"
          @click="handleDelete"
          v-hasPermi="['fill:formtemplate:remove']"
        >删除</el-button>
      </el-col> -->
      <el-col :span="1.5">
        <el-button
          ...
          icon="Download"
          @click="handleExport"
          v-hasPermi="['fill:formtemplate:export']"
        >导出</el-button>
      </el-col>
    ...
    <el-dialog :title="title" v-model="open" width="500px" append-to-body>
      <el-form ref="formtemplateRef" :model="form" :rules="rules" label-width="140px">
        <el-row>
          <el-col :span="24">
            <el-form-item label="数据库schema" prop="tableSchema">
          ...
          <el-col :span="24">
            <el-form-item label="表单类型" prop="formType">
              <el-input v-model="form.formType" placeholder="表单类型（1=单表 2=主子表 3=三层表）" />
            </el-form-item>
          </el-col>
          <el-col :span="24">
            <el-form-item label="所属模块" prop="moduleName">
          ...
          <el-col :span="24">
            <el-form-item label="所在分组" prop="groupName">
              <el-input v-model="form.groupName" placeholder="请输入所在分组" />
            </el-form-item>
          </el-col>
          <el-col :span="24">
            <el-form-item label="显示顺序" prop="orderNum">
...
import { listFormtemplate, getFormtemplate, delFormtemplate, addFormtemplate, updateFormtemplate, syncFormtemplate } from "@/api/fill/formtemplate"
...
function getList() {
  loading.value = true
  listFormtemplate(queryParams.value).then(response => {
    formtemplateList.value = response.rows
...
function reset() {
  ...
  proxy.resetForm("formtemplateRef")
}
...
function handleUpdate(row) {
  ...
  getFormtemplate(_formId).then(response => {
  ...
  proxy.$refs["formtemplateRef"].validate(valid => {
    if (valid) {
      if (form.value.formId != null) {
        updateFormtemplate(form.value).then(() => {
          proxy.$modal.msgSuccess("修改成功")
          open.value = false
          getList()
        })
      } else {
        addFormtemplate(form.value).then(() => {
...
function handleDelete(row) {
  ...
  proxy.$modal.confirm('是否确认删除单模板编号为"' + _formIds + '"的数据项？').then(function() {
    return delFormtemplate(_formIds)
  }).then(() => {
    getList()
...
function handleExport() {
  proxy.download('fill/formtemplate/export', {
    ...queryParams.value
  }, `formtemplate_${new Date().getTime()}.xlsx`)
}

/** 同步业务表：从 information_schema 读取未注册的业务表并自动填充 */
function handleSync() {
  proxy.$modal.confirm('是否确认同步业务表到表单模板表？').then(function() {
