<template>
  <div class="app-container">
    <el-form :model="queryParams" ref="queryRef" :inline="true" v-show="showSearch" label-width="68px">
      ...
      <el-form-item label="冗余：发布版本ID" prop="releaseId">
        <el-input
          v-model="queryParams.releaseId"
          placeholder="请输入冗余：发布版本ID"
          clearable
          @keyup.enter="handleQuery"
        />
      </el-form-item>
      ...
      <el-form-item label="前置运行态菜单ID" prop="predecessorDetailId">
        <el-input
          v-model="queryParams.predecessorDetailId"
          placeholder="请输入前置运行态菜单ID"
          clearable
          @keyup.enter="handleQuery"
        />
      </el-form-item>
      ...
      <el-form-item label="是否在聚合入口卡片显示" prop="operationVisible">
        <el-input
          v-model="queryParams.operationVisible"
          placeholder="请输入是否在聚合入口卡片显示"
          clearable
          @keyup.enter="handleQuery"
        />
      </el-form-item>
    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button
          ...
          icon="Plus"
          @click="handleAdd"
          v-hasPermi="['fill:instance_menu:add']"
        >新增</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="single"
          @click="handleUpdate"
          v-hasPermi="['fill:instance_menu:edit']"
        >修改</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="multiple"
          @click="handleDelete"
          v-hasPermi="['fill:instance_menu:remove']"
        >删除</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          icon="Download"
          @click="handleExport"
          v-hasPermi="['fill:instance_menu:export']"
        >导出</el-button>
      </el-col>
      <right-toolbar v-model:showSearch="showSearch" @queryTable="getList"></right-toolbar>
    </el-row>

    <el-table v-loading="loading" :data="instance_menuList" @selection-change="handleSelectionChange">
      <el-table-column type="selection" width="55" align="center" />
      <el-table-column label="运行态菜单节点主键" align="center" prop="menuId" />
      <el-table-column label="关联批记录实例集ID" align="center" prop="instanceId" />
      <el-table-column label="冗余：发布版本ID" align="center" prop="releaseId" />
      ...
      <el-table-column label="前置运行态菜单ID" align="center" prop="predecessorDetailId" />
      ...
      <el-table-column label="表单生成状态" align="center" prop="instanceStatus" />
      ...
      <el-table-column label="操作" align="center" class-name="small-padding fixed-width">
        <template #default="scope">
          <el-button link type="primary" icon="View" @click="handleViewData(scope.row)" v-hasPermi="['fill:instance_menu:query']">详情</el-button>
          <el-button link type="primary" icon="Edit" @click="handleUpdate(scope.row)" v-hasPermi="['fill:instance_menu:edit']">修改</el-button>
          <el-button link type="primary" icon="Delete" @click="handleDelete(scope.row)" v-hasPermi="['fill:instance_menu:remove']">删除</el-button>
        </template>
      </el-table-column>
    </el-table>
    ...
    <!-- 填报菜单运行态详情抽屉 -->
    <instance_menu-view-drawer ref="instance_menuViewRef" />
    <!-- 添加或修改填报菜单运行态对话框 -->
    <el-dialog :title="title" v-model="open" width="500px" append-to-body>
      <el-form ref="instance_menuRef" :model="form" :rules="rules" label-width="100px">
        <el-row>
          <el-col :span="24">
            <el-form-item label="关联批记录实例集ID" prop="instanceId">
          ...
          <el-col :span="24">
            <el-form-item label="冗余：发布版本ID" prop="releaseId">
              <el-input v-model="form.releaseId" placeholder="请输入冗余：发布版本ID" />
            </el-form-item>
          </el-col>
          <el-col :span="24">
            <el-form-item label="冗余：排产计划ID" prop="planId">
          ...
          <el-col :span="24">
            <el-form-item label="前置运行态菜单ID" prop="predecessorDetailId">
              <el-input v-model="form.predecessorDetailId" placeholder="请输入前置运行态菜单ID" />
            </el-form-item>
          </el-col>
          <el-col :span="24">
            <el-form-item label="操作码" prop="operationCode">
          ...
          <el-col :span="24">
            <el-form-item label="是否在聚合入口卡片显示" prop="operationVisible">
              <el-input v-model="form.operationVisible" placeholder="请输入是否在聚合入口卡片显示" />
            </el-form-item>
          </el-col>
          <el-col :span="24">
            <el-form-item label="业务表主键" prop="businessRecordId">
...
<script setup name="Instance_menu">
import { listInstance_menu, getInstance_menu, delInstance_menu, addInstance_menu, updateInstance_menu } from "@/api/fill/instance_menu"
import Instance_menuViewDrawer from "./view"

const { proxy } = getCurrentInstance()
const { sys_normal_disable } = useDict('sys_normal_disable')

const instance_menuList = ref([])
...
const data = reactive({
  ...
  rules: {
    instanceId: [
      { required: true, message: "关联批记录实例集ID不能为空", trigger: "blur" }
    ],
    menuName: [
...
function getList() {
  loading.value = true
  listInstance_menu(queryParams.value).then(response => {
    instance_menuList.value = response.rows
...
function reset() {
  ...
  proxy.resetForm("instance_menuRef")
}
...
function handleUpdate(row) {
  ...
  getInstance_menu(_menuId).then(response => {
    form.value = response.data
    open.value = true
    title.value = "修改填报菜单运行态"
  })
}

/** 提交按钮 */
function submitForm() {
  proxy.$refs["instance_menuRef"].validate(valid => {
    if (valid) {
      if (form.value.menuId != null) {
        updateInstance_menu(form.value).then(() => {
          proxy.$modal.msgSuccess("修改成功")
          open.value = false
          getList()
        })
      } else {
        addInstance_menu(form.value).then(() => {
...
function handleDelete(row) {
  ...
  proxy.$modal.confirm('是否确认删除填报菜单运行态编号为"' + _menuIds + '"的数据项？').then(function() {
    return delInstance_menu(_menuIds)
  }).then(() => {
    getList()
...
function handleViewData(row) {
  proxy.$refs["instance_menuViewRef"].open(row.menuId)
}

/** 导出按钮操作 */
function handleExport() {
  proxy.download('fill/instance_menu/export', {
    ...queryParams.value
  }, `instance_menu_${new Date().getTime()}.xlsx`)
}
