<template>
  <el-drawer title="填报菜单运行态详情" v-model="visible" direction="rtl" size="60%" append-to-body :before-close="handleClose" class="detail-drawer">
    <div v-loading="loading" class="drawer-content">
      ...
      <el-row :gutter="20" class="mb8">
        <el-col :span="12">
          <div class="info-item">
            <label class="info-label">自定义字段预赋值：</label>
            <span class="info-value plaintext">
              {{ info.customParams }}
            </span>
          </div>
        ...
        <el-col :span="12">
          <div class="info-item">
            <label class="info-label">前置运行态菜单ID：</label>
            <span class="info-value plaintext">
...
<script setup name="Instance_menuViewDrawer">
import { getInstance_menu } from '@/api/fill/instance_menu'
...
const open = async (menuId) => {
  ...
  loading.value = true
  try {
    const res = await getInstance_menu(menuId)
    Object.assign(info, res.data || {})
  } catch (error) {
    console.error('获取填报菜单运行态信息失败:', error)
  } finally {
    loading.value = false
  }
}
