<template>
  <div class="app-container">
    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button
          ...
          icon="Plus"
          @click="handleAdd"
          v-hasPermi="['fill:scheme_design_menu:add']"
        >新增</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
    ...
    <el-table
      v-if="refreshTable"
      v-loading="loading"
      :data="scheme_design_menuList"
      row-key="menuId"
      :default-expand-all="isExpandAll"
      :tree-props="{children: 'children', hasChildren: 'hasChildren'}"
    >
      <el-table-column label="所属方案ID" prop="schemeId" />
      ...
      <el-table-column label="自定义字段预赋值，生成对应空表记录时用" align="center" prop="customParams" />
      <el-table-column label="前置菜单ID" align="center" prop="predecessorDetailId" />
      ...
      <el-table-column label="操作" align="center" class-name="small-padding fixed-width">
        <template #default="scope">
          <el-button link type="primary" icon="View" @click="handleViewData(scope.row)" v-hasPermi="['fill:scheme_design_menu:query']">详情</el-button>
          <el-button link type="primary" icon="Edit" @click="handleUpdate(scope.row)" v-hasPermi="['fill:scheme_design_menu:edit']">修改</el-button>
          <el-button link type="primary" icon="Plus" @click="handleAdd(scope.row)" v-hasPermi="['fill:scheme_design_menu:add']">新增</el-button>
          <el-button link type="primary" icon="Delete" @click="handleDelete(scope.row)" v-hasPermi="['fill:scheme_design_menu:remove']">删除</el-button>
        </template>
      </el-table-column>
    </el-table>

    <!-- 菜单权限设计态详情抽屉 -->
    <scheme_design_menu-view-drawer ref="scheme_design_menuViewRef" />
    <!-- 添加或修改菜单权限设计态对话框 -->
    <el-dialog :title="title" v-model="open" width="500px" append-to-body>
      <el-form ref="scheme_design_menuRef" :model="form" :rules="rules" label-width="100px">
        <el-row>
          <el-col :span="24">
          ...
          <el-col :span="24">
            <el-form-item label="父菜单ID" prop="parentId">
              <el-tree-select
                ...
                :data="scheme_design_menuOptions"
          ...
          <el-col :span="24">
            <el-form-item label="自定义字段预赋值，生成对应空表记录时用" prop="customParams">
              <el-input v-model="form.customParams" type="textarea" placeholder="请输入内容" />
            </el-form-item>
          </el-col>
          <el-col :span="24">
            <el-form-item label="前置菜单ID" prop="predecessorDetailId">
              <el-input v-model="form.predecessorDetailId" placeholder="请输入前置菜单ID" />
            </el-form-item>
          </el-col>
          <el-col :span="24">
            <el-form-item label="操作码" prop="operationCode">
...
<script setup name="Scheme_design_menu">
import { listScheme_design_menu, getScheme_design_menu, delScheme_design_menu, addScheme_design_menu, updateScheme_design_menu } from "@/api/fill/scheme_design_menu"
import Scheme_design_menuViewDrawer from "./view"

const { proxy } = getCurrentInstance()
const { sys_normal_disable, sys_show_hide } = useDict('sys_normal_disable', 'sys_show_hide')

const scheme_design_menuList = ref([])
const scheme_design_menuOptions = ref([])
...
function getList() {
  loading.value = true
  listScheme_design_menu(queryParams.value).then(response => {
    scheme_design_menuList.value = proxy.handleTree(response.data, "menuId", "parentId")
    loading.value = false
  })
}
...
function getTreeselect() {
  listScheme_design_menu().then(response => {
    scheme_design_menuOptions.value = []
    const data = { menuId: 0, menuName: '顶级节点', children: [] }
    data.children = proxy.handleTree(response.data, "menuId", "parentId")
    scheme_design_menuOptions.value.push(data)
  })
}
...
function reset() {
  ...
  proxy.resetForm("scheme_design_menuRef")
}
...
function handleViewData(row) {
  proxy.$refs["scheme_design_menuViewRef"].open(row.menuId)
}
...
async function handleUpdate(row) {
  ...
  getScheme_design_menu(row.menuId).then(response => {
    form.value = response.data
    open.value = true
    title.value = "修改菜单权限设计态"
  })
}

/** 提交按钮 */
function submitForm() {
  proxy.$refs["scheme_design_menuRef"].validate(valid => {
    if (valid) {
      if (form.value.menuId != null) {
        updateScheme_design_menu(form.value).then(() => {
          proxy.$modal.msgSuccess("修改成功")
          open.value = false
          getList()
        })
      } else {
        addScheme_design_menu(form.value).then(() => {
...
function handleDelete(row) {
  proxy.$modal.confirm('是否确认删除菜单权限设计态编号为"' + row.menuId + '"的数据项？').then(function() {
    return delScheme_design_menu(row.menuId)
  }).then(() => {
    getList()
